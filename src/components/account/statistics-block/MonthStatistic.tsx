    ...
    <div className="flex flex-col items-center flex-1 min-w-0">
      <div className='flex flex-col h-40 xs:h-50 sm:h-60 md:h-70 justify-end relative group w-full max-w-16'>
        ...
        <div className='absolute -top-12 left-1/2 transform -translate-x-1/2 bg-gray-800 text-white text-xs py-1 px-2 rounded opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none z-10 whitespace-nowrap'>
          ...
          {predictHeight && (
            <div className='left-1/2'>Прогноз: {predictExpense?.toLocaleString()}</div>
          )}
        ...
        {predictHeight && (
          <div
            ...
            className='bg-green-300 w-full duration-1000 hover:bg-green-600 cursor-pointer rounded-t'
          />
        )}
        ...
        {height === 0 ? (
          <div
            ...
            className='bg-blue-300 w-full duration-1000 hover:bg-blue-600 cursor-pointer rounded-t'
          />
        ) : (
          <div
            style={{
      ...
      <div className='text-gray-400 mt-2 text-center w-full'>
        <span 
          className={"text-xs xs:text-sm block px-1 whitespace-nowrap" + isMobile ? "transform -rotate-45 origin-left" : ""}
          style={isMobile ? { writingMode: 'vertical-lr' } : {}}
        >
          {month}
        </span>
      </div>
